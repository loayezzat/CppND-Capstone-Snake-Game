...
void Game::PlaceFood() {
  ...
  while (true) {
    x = random_w(engine);
    y = random_h(engine);
    // Check food location is not a snake cell or obstacle cell
    if ( (!snake.SnakeCell(x, y)) && ( ! obstacle.isObstacle(x,y)) ) {
      food.x = x;
      food.y = y;
      return;
    }else continue ;
...
void Game::Update() {
  ...
  snake.Update();
  if (snake.level != obstacle.getLevel()) {obstacle.obstacleUpdate(snake.level) ; }
